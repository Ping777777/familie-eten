...
export default function RecipeLibrary({ recipes, onAdd, onDelete, onUpdate, saveFailed, onDismissSaveFailed, searchQuery = "", editListMode, newRecipeKey, archiveOpenKey, editRequestRecipe, onEditRequestConsumed }) {
  ...
  useEffect(() => {
    if (archiveOpenKey > 0) setArchiveOpen(true);
  }, [archiveOpenKey]);

  useEffect(() => {
    if (editRequestRecipe) { setEditingRecipe(editRequestRecipe); onEditRequestConsumed?.(); }
  }, [editRequestRecipe]);
      ...
      <div className="recipe-grid">
        {filtered.map((recipe) => (
          <RecipeCard
            ...
            onFavorite={() => onUpdate({ ...recipe, favorite: !recipe.favorite })}
...
function RecipeCard({ recipe, expanded, onToggle, onEdit, onArchive, onDelete, onFavorite, archiveBtn, dimmed, editMode }) {
  const { t, lang } = useLanguage();
      ...
      <div className="rc-card-body" onClick={(e) => { if (editMode) onEdit(e); else onToggle(); }}>
        <div className="rc-header">
          ...
          <div className="rc-header-right">
            {onFavorite && (
              <button
                className={`rc-favorite-btn${recipe.favorite ? " rc-favorite-btn--on" : ""}`}
                onClick={(e) => { e.stopPropagation(); onFavorite(); }}
              >
                <svg width="15" height="15" viewBox="0 0 24 24" fill={recipe.favorite ? "currentColor" : "none"} stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
